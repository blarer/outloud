...
the OSC cursor color, the tmux widget, the TUI header, and `outloud status
...
stateDiagram-v2
    ...
    Idle --> Listening: hotkey down / "hey outloud"
...
Voice activation ("hey outloud") and its arming rules, "run it / send it" for
...
output, log locations). Every advanced setting has a `outloud set` key with the
...
The dictionary is a competitive front: OutLoud caps free users at 5 entries and
...
- Every setting change applies live, no restart, and is reflected in
  `outloud status --json` immediately, because the TUI/CLI and GUI are equal
