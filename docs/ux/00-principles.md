...
- **The invisible tool must still be findable.** Invisibility is a default,
  ...
  `aqua` CLI on headless systems) reaches status, settings, and help. If the
...
TDT finalizing) fits inside it. Aqua's cloud round-trip is ~450ms to insert.
...
- **Measure in the product, not the lab.** The per-stage timing breakdown the
  ...
  (overlay hover / `aqua doctor`). Users on old hardware deserve to see
  *which* stage is slow and what model tier would fix it.

## 3. Trust and privacy are product surfaces, not policy pages

Local-first is the entire differentiator against Aqua and Wispr Flow. Aqua is
...
- **Show what was heard, own what was written.** History (transcripts and
  ...
  Aqua's history cannot be disabled; ours can, per-app or globally, and the
