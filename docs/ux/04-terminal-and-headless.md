...
OutLoud works in "even a raw terminal" by pasting keystrokes. Nobody, closed or
...
below, and the active tier is visible in `outloud doctor`:
...
- **Nothing we inject ever ends with Enter by itself.** Dictation composes
  ...
  setting, mirroring OutLoud's "Send It" but gated because terminals execute
...
maintains a tiny local socket (`$XDG_RUNTIME_DIR/outloud/shell.sock`, or a
...
A shipped tmux plugin (`outloud.tmux`, TPM-installable) adds a status segment
...
`outloud shell-install --ssh user@host` (appends the plugin line to the remote
...
**B. Fully headless (`outloud`).** On a machine with no GUI at all, dictating
*at* that machine's console: a user with a USB mic on a TTY, or audio
forwarded from a thin client. `outloud` is the same engine as the desktop app
...
**CLI (scriptable truth):** `outloud status`, `outloud doctor`, `outloud listen`,
`outloud undo`, `outloud models`, `outloud set <key> <value>`, all with `--json`.
The CLI is the API; the TUI and GUI are both clients of it.

**TUI (`outloud tui`):** a full-screen ratatui-style surface for humans:
