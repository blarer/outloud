...
1. **Explicit selection.** If the focused field has a selection when the key
   ...
   path and matches Aqua's Edit Mode gesture (select, hold, speak), so
...
There is deliberately **no command prefix word**. Aqua's Edit Mode uses no
...
holding the key, matching Aqua's stackable-undo behavior head-on.
...
- **No auto-applied freeform.** However good the model gets, generated text
  ...
  your text without you seeing it first", which is a sentence Aqua cannot say
