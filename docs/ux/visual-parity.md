...
like Aqua Voice, to the point that someone who used Aqua yesterday feels at
...
| Source | What it gave |
|---|---|
| `withaqua.com` stylesheet (`/_next/static/css/eefa388e02313efb.css`) | The **exact design tokens**: `.bg-cobalt`, `.bg-ink`, `.bg-aqua`, … as literal RGB values, plus the font stack and the radius scale |
...
| `aquavoice.com/changelog` | Design-system history: 0.15.1 "Refreshed the desktop app with Aqua's updated design system… Updated the loading and sign-in screens with the new Aqua orb"; 0.9.5 "Beginnings of a UI refresh"; 0.11.5 "The microphone UI no longer jumps around when recording"; 0.11.9 "Fixed Floating Bar positioning issue when in fullscreen" |
...
- I did not install the Aqua macOS app, so I have **no direct screenshot of
...
observed, not a measurement. If someone installs Aqua later, the inferred
...
| `aqua` | `#67BEFF` | Lighter brand blue |
| `aqua-deep` | `#219AF7` | Saturated blue for emphasis on light surfaces |
| `aqua-pale` | `#A7DAFC` | Palest blue; reads as "waiting" |
...
| What | Image |
|---|---|
| Aqua brand orb (`orb-128.png`) | ![aqua orb](img/aqua-orb.png) |
| 9to5Mac press composite | ![aqua press](img/aqua-press-9to5.jpg) |
...
Aqua carefully avoids. *Fix: `theme::TAIL_MONOSPACE = false`, 12.5pt.*

**5. No fade.** The card blinks in and out. Aqua's UI reads as smooth. The
...
| Transcribing | `#67BEFF` (aqua) | one step lighter = progress, not a new condition |
| ModelLoading | `#A7DAFC` (aqua-pale) | plus the 0.6Hz pulse |
| Error | `#FF4A3D` (ember) | ours; Aqua publishes no failure token |
...
Aqua's own sections, from their guide navigation: General, Keybindings,
...
**Privacy is a top-level row and is where we deliberately diverge.** Aqua has
...
## Where Aqua's choice is worse, and we should not copy it
...
an explicit preference for people migrating from Aqua, and as a stable
