...
and the Edit Mode and Send It announcement posts. OutLoud's shipping version at
time of writing is **0.18.0** (26 July 2026).

This document has three parts: what OutLoud actually ships, an honest gap analysis
against this repository with file-level citations, and a prioritized build
order.

The framing that matters throughout: **OutLoud is a cloud product with a
...
OutLoud explicitly does *not* claim one mode is correct: "Instant Mode has the
lowest latency and is great for short clips or chaining OutLoud many times in a
...
### 1.3 The command vocabulary OutLoud teaches
...
**Replacement, implicit — the big one.** OutLoud's headline claim is that *no
command words are needed at all*:

> "If what you say is clearly a corrected version of what you selected, OutLoud
...
`"send it"`; OutLoud strips the command from the transcript, inserts the message,
...
- **Positional rule**: the command must be the *last* thing said. "Tell them
  ...
  sends. OutLoud explicitly says it "reads the whole thing you said and works out
...
- Disambiguation is delegated to the editor's own autocomplete; OutLoud emits the
  tag and lets the IDE resolve it.

**Punctuation.** Notably, OutLoud teaches *no* punctuation commands at all. There
...
| **Custom Instructions** | Free-text natural-language style rules, ChatGPT-style. OutLoud's own advice: write rules as a list, and **always include a good-output and bad-output example**, because examples are what make the model comply. | Pro. |
...
- **Menu bar resident** with an optional Dock icon. Two separate quit paths
  ("Quit Aqua Voice (UI)" vs "Quit OutLoud Completely") which confuse users badly
...
- **Stats card / dashboard**: time saved, WPM, activity graph, shareable.
  "OutLoud Wrapped" year-in-review.
...
### 1.6 What OutLoud cannot do

Stated plainly in their own FAQ and comparison pages:

- **No offline mode.** "Yes, OutLoud is cloud-based and needs a connection."
...
| OutLoud feature | Us | Where |
|---|---|---|
| Push-to-talk, hold-key dictation | ✅ | `crates/hotkey`, `crates/outloud/src/pipeline.rs`; 189ms measured |
| Instant Mode (commit on release) | ✅ | `insertion.mode = "on-release"`, `crates/config/src/schema.rs:189` |
| Realtime / streaming injection | 🟡 | `crates/stream` is fully built (`session.rs`, `undo.rs`) but **`outloud/Cargo.toml` does not depend on `stream`**. The setting exists in the menu; the behaviour does not. |
| Hands-free latch (double-press / hold+Space) | ❌ | No latch mode anywhere. `silence-timeout-ms` exists in schema only. |
| System-wide injection into any app | ✅ | `crates/text-target` with six tiers (`Accessibility`, `InputMethod`, `SyntheticKeys`, `Clipboard`, `TerminalNative`, `Headless`) — strictly more tiers than OutLoud documents |
...
| OutLoud feature | Us | Where |
|---|---|---|
| Selection + same hotkey → edit, no mode switch | ✅ | `crates/outloud/src/pipeline.rs`, 131ms measured |
| `"change X to Y"` | ✅ | `crates/edit-intent/src/lib.rs:52` — and we accept **four** shapes to Aqua Voice's one (`change…to`, `replace…with`, `make…into`, `swap…for`) |
| Delete | ✅ | `edit-intent` line 74: `delete`, `remove`, `get rid of`, `scratch` |
| Append | ✅ | line 86: `append`, `add`, `also add` |
| Recase (upper/lower/title/sentence) | ✅ | line 108–111. OutLoud does not document these at all. |
| **Command-word-free replacement** ("say the corrected version") | ❌ | Our parser falls through to `Freeform` and refuses. This is OutLoud's single most-promoted Edit Mode behaviour. |
| **Spoken undo** (`"undo that"` / `"go back one step"` / `"go back to the original"`) | 🟡 | `crates/stream/src/undo.rs` implements the ring with a correct staleness check, but no parser verb reaches it and `outloud` does not link `stream`. |
| Spelling correction (`"It's T-O-N-I"`) | ❌ | No letter-sequence handling |
| `"translate to X"` | 🚫 | Needs a multilingual LLM |
| `"fix the grammar"` / `"make it shorter"` / `"abbreviate"` / `"more formal"` | 🟡 → 🚫 | `crates/llm` exists complete (`transform`, `prompt`, `guardrail`, `sanitize`, `llama_backend`) but **`outloud` has no `llm` dependency**; `Outcome::FreeformUnsupported` in `pipeline.rs:364` is the honest refusal. Wiring it makes these work locally at Qwen3-1.7B quality, which is below OutLoud's. |
...
| Terminal command-line edit-by-voice | ✅ **unique** | `crates/shell-bridge`; OutLoud cannot do this |

### 2.3 Customization

| OutLoud feature | Us | Where |
|---|---|---|
| Dictionary / custom terms | 🟡 | `crates/config/src/vocab.rs` is excellent and **unlimited** (OutLoud caps at 800): bias terms, replacements, per-entry `[case]` / `[strip-punct]` flags, plus fuzzy correction via `fuzzy.rs` ("cube cuddle" → `kubectl`). **But `vocab::` is referenced nowhere outside `crates/config`** — it is not wired into the pipeline. |
...
| Deep Context (screen reading for term bias) | 🟡 **and we can beat it** | We already read the focused field via AX for editing. Feeding the surrounding field text into vocabulary bias is a strictly local version of Deep Context, and unlike OutLoud it never leaves the machine. |

### 2.4 System surfaces

| OutLoud feature | Us | Where |
|---|---|---|
| Menu bar status item + click menu | ✅ | `crates/overlay/src/menu.rs`, `status_item.rs`, `crates/outloud/src/menubar.rs`. Glyph per state, settings write through `config::update_file`. |
...
| Onboarding wizard with verified permissions | 🟡 **design beats theirs** | `docs/ux/01-onboarding.md` specifies a probe-based checklist that is better than anything OutLoud ships; `scripts/grant-accessibility.sh` and `crates/diag` exist; the wizard itself does not. |
...
| Multiple hotkeys / chord recorder | 🟡 | `crates/hotkey` binds one chord; `conflict.rs` probes conflicts (better than OutLoud's "try it in different apps" advice). Menu offers four presets, no recorder, no alternates. |
| Launch at login | 🟡 | Key + menu row exist; no `SMAppService` registration |
| Stats / dashboard | ❌ | — |
| Diagnostics | ✅ **better** | `scripts/doctor.sh`, `crates/diag`, menu "Run diagnostics…". OutLoud's equivalent is "zip your logs and email us." |
...
injection. This single change delivers OutLoud's Dictionary *and* Replacements
...
profiles for terminals, Slack/Discord/iMessage (casual-lowercase — OutLoud's
...
quality is below OutLoud's; the guardrail already refuses bad generations rather
than shipping them.
...
correct. OutLoud documents exactly three of these phrasings; matching them costs
...
has the similarity machinery. This is the behaviour OutLoud promotes hardest, it
...
Cap edits at 6,000 characters (OutLoud's exact number) and fall back to dictation.
Detect address bars and search fields and always dictate there. Both are
one-line product decisions OutLoud learned the hard way and published; taking them
...
hotkey later. OutLoud shipped this as a named feature and users asked for it to
...
`docs/ux/01-onboarding.md` already specifies it in full. OutLoud's permission
...
Transcripts plus audio on disk, with replay, re-transcribe, and copy. OutLoud
...
OutLoud allows 5 bindings per action and had to add "Add Alternative" because
...
execute things. OutLoud's version relies on an LLM deciding whether the user
...
2. Command-word-free replacement in `edit-intent` (OutLoud's headline Edit Mode
...
Collected verbatim or near-verbatim from OutLoud's documentation and from the
phrasings their users are trained on. Ordered by expected frequency.

```
# undo (OutLoud documents the first three exactly)
undo that
go back one step
go back to the original
undo
...
# recasing, already covered (OutLoud documents none of these)
