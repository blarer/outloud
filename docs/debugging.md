...
| Worked yesterday, broken after `cargo build`, toggle still reads ON | Ad-hoc signature: TCC pins the grant to the cdhash, every rebuild silently revokes it | `tccutil reset Accessibility dev.aquaoss.spike`, re-grant. Long term: Developer ID certificate, which pins to team id instead |
