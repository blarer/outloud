...
# its output to AQUA_SPIKE_LOG and this script tails it, same contract as
# scripts/run.sh.
#
...
APP_NAME="AquaDoctor"
BUNDLE_ID="dev.aquaoss.doctor"
APP_DIR="$ROOT/dist/$APP_NAME.app"
LOG="${TMPDIR:-/tmp}/aqua-doctor-$$.log"
...
: > "$LOG"
# AQUA_LAUNCHED_VIA_LS tells the doctor it is genuinely under LaunchServices:
# `open --env` leaks the caller's TERM through, which would otherwise trip the
# shell-launch heuristic even in a correct launch.
open -a "$APP_DIR" --env "AQUA_SPIKE_LOG=$LOG" --env "AQUA_LAUNCHED_VIA_LS=1" --args "$@"
