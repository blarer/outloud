...
// Everything below touches `ax_edit::synth`, which lib.rs only declares on
// macOS. `cargo clippy --all-targets` builds this example on every platform,
// so an ungated body breaks the Linux job (same bug as type_at.rs). The gate
// is `target_os`, not a Cargo feature, because that is the axis the library
// splits on: `required-features` cannot express "macOS only".
#[cfg(target_os = "macos")]
mod mac {
...
/// Off macOS there is nothing to sweep: no synthetic-keys backend exists.
#[cfg(not(target_os = "macos"))]
fn main() {
    eprintln!("pacing_floor is macOS-only: ax_edit::synth has no backend on this platform");
    std::process::exit(1);
}
